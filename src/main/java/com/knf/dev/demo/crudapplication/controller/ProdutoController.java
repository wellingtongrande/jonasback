...
public class ProdutoController {
	...
	public ResponseEntity<Produto> getProdutoById(@PathVariable Long id) {
		Produto p = produtoRepository.findById(id)
				.orElseThrow(() -> new ResourceNotFoundException("Id  do produto não existe :" + id));
	...
	public ResponseEntity<Produto> updatedProduct(@PathVariable Long id,
			@RequestBody Produto produtoDetails) {
		Produto p = produtoRepository.findById(id)
				.orElseThrow(() -> new ResourceNotFoundException("Id do produto não existe :" + id));

		p.setDescricao(produtoDetails.getDescricao());
		p.setImg(produtoDetails.getImg());
